...
link *initLink(void)
{
    link *p = (link *)kmalloc(sizeof(link), GFP_KERNEL); // 创建一个头指针（不包含实际值）（就是个空结点）
...
static int hello_init(void) // static使得该文件以外无法访问
{
    ...
    printk(KERN_INFO "在第4的位置插入元素6:\n");
    p = insertElem(p, 6, 4);
    display(p);

    printk(KERN_INFO "删除元素2:\n");
    p = delElem(p, 2);
    display(p);
...
// 宏 声明可传入参数
module_param(num, int, S_IRUGO);  // S_IRUGO 文件系统所对应的文件节点的属性
